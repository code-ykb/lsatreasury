...
function attachAdjustmentsHandlers() {
  ...
  obPost.addEventListener("click", () => {
    ...
    const derivedFund = deriveFundForEntry(
      preset === "FUND_BANK" ? obFund.value : "",
      debitCode,
      creditCode
    );
    const row = {
    ...
    postJournalWithTag(
      { date, fund: derivedFund, desc, debit: debitCode, credit: creditCode, amount: amt },
      tag,
      "OB"
    );
  ...
  adjPost.addEventListener("click", () => {
    ...
    const derivedFund = deriveFundForEntry(fund, debit, credit);
    const row = { id: tag, date, fund: derivedFund, desc, debit, credit, amount: amt };
    const rows = adjRows();
    rows.push(row);
    saveAdjRows(rows);

    postJournalWithTag(
      { date, fund: derivedFund, desc, debit, credit, amount: amt },
      tag,
      "ADJ"
    );
